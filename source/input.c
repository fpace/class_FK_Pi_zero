...
int input_read_parameters(
  ...
  /** - Background paramters for F(K) e.g. Omega_0_F(K), w_F(K) starts here */

  class_read_double("w_fk",pba->w_fk);
  //zclass_read_double("F0",pba->P2);
  class_read_double("P1",pba->P1);
  class_read_double("P2",pba->P2);   
  class_read_double("P3",pba->P3);  
  class_read_double("P4",pba->P4);  
  //class_read_double("c1",pba->c1);
  //class_read_double("c2",pba->c2);
  //class_read_double("c3",pba->c3);
  //class_read_double("c4",pba->c4);
  //class_read_double("c13",pba->c13);
  //class_read_double("c14",pba->c14);
  //class_read_double("alpha_c",pba->alpha_c);
  class_read_double("a_start_fk",pba->a_start_fk);
  class_read_double("k_max_fk",ppt->k_max_fk);
  class_read_double("A_lens",ple->A_lens);

  /** - Background paramters for F(K) e.g. Omega_0_F(K), w_F(K) ends here */
  ...
  class_call(parser_read_double(pfc,"Omega_scf",&param3,&flag3,errmsg),
             errmsg,
             errmsg);
  class_call(parser_read_double(pfc,"Omega_fk",&param4,&flag4,errmsg), //F(K) here
             errmsg,
             errmsg);
  ...
  */
  ...
  if (flag4 == _TRUE_){
    pba->Omega0_fk = param4;
    Omega_tot += pba->Omega0_fk;
  }
  /* Step 2 */
  ...
  else if (flag4 == _FALSE_) // Fill up with F(K) fluid
    pba->Omega0_fk = 1. - pba->Omega0_k - Omega_tot;
...
int input_default_params(
  ...
  //pba->c3 = 0.; //F(K) here
  //pba->c4 = 0.; //F(K) here
  //pba->c13 = pba->c1 + pba->c3;
  //pba->c14 = pba->c1 - pba->c4;
  //pba->alpha_c = pba->c1 + 3.0*pba->c2 + pba->c3;
  pba->a_start_fk = 0.01; //F(K) here
  ppt->k_max_fk = 0.; //F(K) here
