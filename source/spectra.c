...
int spectra_indices(
  ...
  /*F(K) starts here*/
  class_define_index(psp->index_tr_lensing_potential_fk,ppt->has_source_lensing_potential_fk,index_tr,1);
  class_define_index(psp->index_tr_wPi_fk,ppt->has_source_wPi_fk,index_tr,1);
  class_define_index(psp->index_tr_wGamma_fk,ppt->has_source_wGamma_fk,index_tr,1);
  class_define_index(psp->index_tr_Delta_m,ppt->has_source_Delta_m,index_tr,1);
  class_define_index(psp->index_tr_Delta_fk,ppt->has_source_Delta_fk,index_tr,1);
  class_define_index(psp->index_tr_mu_fk,ppt->has_source_Delta_fk,index_tr,1);
  class_define_index(psp->index_tr_eta_fk,ppt->has_source_Delta_fk,index_tr,1);

  class_define_index(psp->index_tr_wPi_m,ppt->has_source_wPi_fk,index_tr,1);
  class_define_index(psp->index_tr_wGamma_m,ppt->has_source_wGamma_fk,index_tr,1);
  
  class_define_index(psp->index_tr_Delta_m_nl_factor,ppt->has_source_Delta_m_nl_factor,index_tr,1);
  /*F(K) ends here*/
...
int spectra_pk(
  ...
  //class_test((psp->sigma8) > 1000000.0, psp->error_message, "sigma8 too large");
  ...
  for (index_tau=0 ; index_tau < psp->ln_tau_size; index_tau++) {
    ...
    for (index_k=0; index_k<psp->ln_k_size; index_k++) {
      ...
      for (index_ic = 0; index_ic < psp->ic_size[index_md]; index_ic++) {
       ...
       /*F(K) starts here*/
       if (ppt->has_source_lensing_potential_fk == _TRUE_) {

          psp->matter_transfer[((index_tau*psp->ln_k_size + index_k) * psp->ic_size[index_md] + index_ic) * psp->tr_size + psp->index_tr_lensing_potential_fk] = ppt->sources[index_md]
            [index_ic * ppt->tp_size[index_md] + ppt->index_tp_lensing_potential_fk]
            [(index_tau-psp->ln_tau_size+ppt->tau_size) * ppt->k_size[index_md] + index_k];

        }

        if (ppt->has_source_wPi_fk == _TRUE_) {

          psp->matter_transfer[((index_tau*psp->ln_k_size + index_k) * psp->ic_size[index_md] + index_ic) * psp->tr_size + psp->index_tr_wPi_fk] = ppt->sources[index_md]
            [index_ic * ppt->tp_size[index_md] + ppt->index_tp_wPi_fk]
            [(index_tau-psp->ln_tau_size+ppt->tau_size) * ppt->k_size[index_md] + index_k];

        }
	
        if (ppt->has_source_wGamma_fk == _TRUE_) {

          psp->matter_transfer[((index_tau*psp->ln_k_size + index_k) * psp->ic_size[index_md] + index_ic) * psp->tr_size + psp->index_tr_wGamma_fk] = ppt->sources[index_md]
            [index_ic * ppt->tp_size[index_md] + ppt->index_tp_wGamma_fk]
            [(index_tau-psp->ln_tau_size+ppt->tau_size) * ppt->k_size[index_md] + index_k];
        ...
        if (ppt->has_source_Delta_m == _TRUE_) {
          ...
          psp->matter_transfer[((index_tau*psp->ln_k_size + index_k) * psp->ic_size[index_md] + index_ic) * psp->tr_size + psp->index_tr_Delta_m] = ppt->sources[index_md]
            [index_ic * ppt->tp_size[index_md] + ppt->index_tp_Delta_m]
            [(index_tau-psp->ln_tau_size+ppt->tau_size) * ppt->k_size[index_md] + index_k];

         psp->matter_transfer[((index_tau*psp->ln_k_size + index_k) * psp->ic_size[index_md] + index_ic) * psp->tr_size + psp->index_tr_Delta_m_nl_factor] = ppt->sources[index_md]
            [index_ic * ppt->tp_size[index_md] + ppt->index_tp_Delta_m_nl_factor]
            [(index_tau-psp->ln_tau_size+ppt->tau_size) * ppt->k_size[index_md] + index_k];

	  

        }
        if (ppt->has_source_Delta_fk == _TRUE_) {

          psp->matter_transfer[((index_tau*psp->ln_k_size + index_k) * psp->ic_size[index_md] + index_ic) * psp->tr_size + psp->index_tr_Delta_fk] = ppt->sources[index_md]
            [index_ic * ppt->tp_size[index_md] + ppt->index_tp_Delta_fk]
            [(index_tau-psp->ln_tau_size+ppt->tau_size) * ppt->k_size[index_md] + index_k];



          psp->matter_transfer[((index_tau*psp->ln_k_size + index_k) * psp->ic_size[index_md] + index_ic) * psp->tr_size + psp->index_tr_mu_fk] = ppt->sources[index_md]
            [index_ic * ppt->tp_size[index_md] + ppt->index_tp_mu_fk]
            [(index_tau-psp->ln_tau_size+ppt->tau_size) * ppt->k_size[index_md] + index_k];


          psp->matter_transfer[((index_tau*psp->ln_k_size + index_k) * psp->ic_size[index_md] + index_ic) * psp->tr_size + psp->index_tr_eta_fk] = ppt->sources[index_md]
            [index_ic * ppt->tp_size[index_md] + ppt->index_tp_eta_fk]
            [(index_tau-psp->ln_tau_size+ppt->tau_size) * ppt->k_size[index_md] + index_k];	  


	  
          psp->matter_transfer[((index_tau*psp->ln_k_size + index_k) * psp->ic_size[index_md] + index_ic) * psp->tr_size + psp->index_tr_wGamma_m] = ppt->sources[index_md]
            [index_ic * ppt->tp_size[index_md] + ppt->index_tp_wGamma_m]
            [(index_tau-psp->ln_tau_size+ppt->tau_size) * ppt->k_size[index_md] + index_k];


          psp->matter_transfer[((index_tau*psp->ln_k_size + index_k) * psp->ic_size[index_md] + index_ic) * psp->tr_size + psp->index_tr_wPi_m] = ppt->sources[index_md]
            [index_ic * ppt->tp_size[index_md] + ppt->index_tp_wPi_m]
            [(index_tau-psp->ln_tau_size+ppt->tau_size) * ppt->k_size[index_md] + index_k];	  
...
int spectra_output_tk_titles(struct background *pba,
  ...
  if (output_format == class_format) {
    ...
    if (ppt->has_density_transfers == _TRUE_) {
      ...
      class_store_columntitle(titles,"WL",ppt->has_source_lensing_potential_fk); /*F(K) starts here*/
      class_store_columntitle(titles,"Om_fk*wPi_fk",ppt->has_source_wPi_fk);
      class_store_columntitle(titles,"Om_fk*wGamma_fk",ppt->has_source_wGamma_fk);
      class_store_columntitle(titles,"Om_m*Delta_m",ppt->has_source_Delta_m);
      class_store_columntitle(titles,"Om_fk*Delta_fk",ppt->has_source_Delta_fk);
      class_store_columntitle(titles,"Delta_m_nl_factor",ppt->has_source_Delta_m_nl_factor);
      class_store_columntitle(titles,"mu_fk",ppt->has_source_Delta_fk);
      class_store_columntitle(titles,"eta_fk",ppt->has_source_Delta_fk);
        class_store_columntitle(titles,"Omega_m*wPi_m",ppt->has_source_Delta_fk);
       class_store_columntitle(titles,"Omega_m*wGamma_m",ppt->has_source_Delta_fk); /*F(K) ends here*/
      ...
      for (index_k=0; index_k<psp->ln_k_size; index_k++) {
        ...
        if (output_format == class_format) {
          ...
          if (ppt->has_density_transfers == _TRUE_) {
            ...
            class_store_double(dataptr,tk[psp->index_tr_lensing_potential_fk],ppt->has_source_lensing_potential_fk,storeidx); //F(K) starts here
	    
            class_store_double(dataptr,tk[psp->index_tr_wPi_fk],ppt->has_source_wPi_fk,storeidx);
            class_store_double(dataptr,tk[psp->index_tr_wGamma_fk],ppt->has_source_wGamma_fk,storeidx);
            class_store_double(dataptr,tk[psp->index_tr_Delta_m],ppt->has_source_Delta_m,storeidx);
             class_store_double(dataptr,tk[psp->index_tr_Delta_fk],ppt->has_source_Delta_fk,storeidx);
            class_store_double(dataptr,tk[psp->index_tr_Delta_m_nl_factor],ppt->has_source_Delta_m_nl_factor,storeidx);
            class_store_double(dataptr,tk[psp->index_tr_mu_fk],ppt->has_source_Delta_m_nl_factor,storeidx);
            class_store_double(dataptr,tk[psp->index_tr_eta_fk],ppt->has_source_Delta_m_nl_factor,storeidx);
            class_store_double(dataptr,tk[psp->index_tr_wPi_m],ppt->has_source_Delta_m_nl_factor,storeidx);
            class_store_double(dataptr,tk[psp->index_tr_wGamma_m],ppt->has_source_Delta_m_nl_factor,storeidx); 
            //F(K) ends here
          }
          if (ppt->has_velocity_transfers == _TRUE_) {
